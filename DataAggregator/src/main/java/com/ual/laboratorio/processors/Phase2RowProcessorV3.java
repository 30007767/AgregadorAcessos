...
@Slf4j
@Component
public class Phase2RowProcessorV3 extends RowProcessor {
    ...
    public MediaAcesso getCourseAccess() throws ParseException {
        ...
        MediaAcesso acesso=null;
            ...
            Number number = format.parse(numberCorrection(row.get(10).getText()));
            acesso = MediaAcesso.builder()
                    ...
                    .vagasRecolocados(Integer.parseInt(numberCorrection(row.get(6).getText())))
                    .colocados(Integer.parseInt(numberCorrection(row.get(7).getText())))
                    .colocadosVagaAdicional(Integer.parseInt(numberCorrection(row.get(8).getText())))
                    .colocadosSemClassificacao(Integer.parseInt(numberCorrection(row.get(9).getText())))
                    .notaUltimoColocado(number.doubleValue())
                    .vagasSobrantes(Integer.parseInt(numberCorrection(row.get(11).getText())))
                    .build();
    ...
    public boolean checkParserCompatibility(Configuracao configuracao){
        return  configuracao.getAno()>2018 && configuracao.getFase()==2;
    }
}
